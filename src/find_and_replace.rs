...
fn print_help(){
    ...
    eprintln!("USAGE - <TARGET STRING>  <REPLACEMENT STRING>");
}
fn replace(target:&str,rep:&str,data:&str)->Result<String,regex::Error>{
    let regex=Regex::new(target)?;
    Ok(regex.replace_all(data, rep).to_string())
}
fn parse_arg()->Arguments{
    let args:Vec<String>=env::args().skip(1).collect();
    if args.len() !=4{
       print_help();
       eprintln!("{} - Wrong number of arguments added expected four arguments,but we got {}","Error".red().bold(),args.len());
       std::process::exit(1)
    }
    Arguments { pattern: args[0].clone()
        , replace: args[1].clone(), input_file:args[2].clone(), output_file: args[3].clone() }
}
...
let replace_data= match replace(&args.pattern,&args.replace,&data){
    Ok(d)=>d,
    Err(e)=>{
        eprintln!("{} failed to replace from file{}:{:?}","Error".red().bold(),args.output_file,e);
            std::process::exit(1);   
    }
};
match fs::write(&args.output_file
,&replace_data) {
    Ok(_)=>{},
    Err(e)=>{
